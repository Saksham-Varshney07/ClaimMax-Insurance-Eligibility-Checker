...
const QRScanner = {
     ...
     */
    async scan(source: HTMLImageElement | HTMLCanvasElement | string): Promise<QRScanResult> {
     ...
     */
    async fallbackScan(canvas: HTMLCanvasElement): Promise<Partial<QRScanResult>> {
        const result: Partial<QRScanResult> = {
            found: false,
            text: null,
            confidence: 0,
            position: null
     ...
     */
    detectFinderPatterns(grayscale: Uint8Array, width: number, height: number): PatternPoint[] {
        const patterns: PatternPoint[] = [];
        const threshold = 128;
     ...
     */
    isFinderPattern(count: number[]): boolean {
     ...
     */
    clusterPatterns(patterns: PatternPoint[]): PatternPoint[] {
        const clustered: PatternPoint[] = [];
     ...
     */
    isPMJAYQR(qrText: string | null): boolean {
     ...
     */
    parsePMJAYQR(qrText: string): ParsedQRData {
        try {
            // Try JSON first
...
export { QRScanner, QRScanResult, PatternPoint, ParsedQRData };
